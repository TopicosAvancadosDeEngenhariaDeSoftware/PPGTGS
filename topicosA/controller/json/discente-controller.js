...
exports.cadastrarDiscente = (req, res, next) => {
    ...
    req.assert('nome', "Verifique seu nome").isLength({
        min: 1,
        max: 40
    });
    req.assert('sobrenome', 'Sobrenome é obrigatório').notEmpty();
    req.assert('sobrenome', "Verifique seu sobrenome").isLength({
        min: 1,
        max: 60
    });
    ...
    if (req.body.id_nacionalidade != undefined && req.body.id_nacionalidade != null && parseInt(req.body.id_nacionalidade) == 1) {
        ...
        req.assert('endereco_cep', 'Cep é obrigatório').notEmpty().isLength({
            min: 1,
            max: 20
        });
        req.assert('endereco_bairro', 'Bairro é obrigatório').notEmpty().isLength({
            min: 1,
            max: 40
        });
        req.assert('endereco_logradouro', 'Logradouro é obrigatório').notEmpty().isLength({
            min: 1,
            max: 60
        });
        req.assert('endereco_num_residencia', 'Número da residência é obrigatório').notEmpty().isLength({
            min: 1,
            max: 20
        });

    } else if (req.body.id_nacionalidade != undefined && req.body.id_nacionalidade != null && parseInt(req.body.id_nacionalidade) != 1) {
    ...
    req.assert('senha', 'Verifique a sua senha').notEmpty().isLength({
        min: 5,
        max: 60
    });
    req.assert('senha_conf', 'Verifique a sua confirmação de senha').notEmpty().isLength({
        min: 5,
        max: 60
    });
    ...
    verifica_discente.recuperarDiscentePorEmail(email, (error, result_existe_discente) => {
        if (error) {
            ...
            if (result_existe_discente == null) {
                ...
                if (req.body.id_nacionalidade == 1) {
                    //PEGAR ID LOGRADOURO
                    verifica_logradouro.recuperarLogradouroPorNome(logradouro, (error, result_logradouro) => {
                        ...
                        } else if(result_logradouro == null || result_logradouro == undefined) { // não existe logradouro
                ...
                } else { //Not Brasileiro, no need for endereco
...
exports.editarDiscente = (req, res, next) => {
    ...
    if (req.body.endereco_id_pais != undefined && req.body.endereco_id_pais != null && parseInt(req.body.endereco_id_pais) == 1) {
        ...
        resultado: null,
        erro: erros_mensagem.erro_usuario_permissao
    });
}
